    ...
    >
      <style>{`
        ...
        @media(max-width:768px){
          ...
          .hero-badges{flex-direction:column!important;gap:0.75rem!important;}
          .hero-trust{flex-wrap:wrap!important;gap:0.5rem!important;}
        }
      `}</style>

      {/* ── Deep space background layers ── */}
      <div style={{ position:'absolute', inset:0, background:'radial-gradient(ellipse 80% 60% at 70% 30%, rgba(249,115,22,0.08) 0%, transparent 60%), radial-gradient(ellipse 60% 80% at 20% 80%, rgba(30,60,200,0.12) 0%, transparent 60%), radial-gradient(ellipse 40% 40% at 50% 50%, rgba(249,115,22,0.04) 0%, transparent 70%)' }} />
      ...
      <div style={{ position:'absolute', width:300, height:300, borderRadius:'50%', background:'radial-gradient(circle, rgba(249,115,22,0.07) 0%, transparent 70%)', top:'50%', left:'50%', transform:'translate(-50%,-50%)', pointerEvents:'none' }} />
      ...
      {mounted && PARTICLES.map((p, i) => (
        <div key={i} style={{
          ...
          background:i % 3 === 0 ? '#F97316' : i % 3 === 1 ? '#6080FA' : 'rgba(255,255,255,0.6)',
      ...
      <div style={{ position:'relative', zIndex:10, maxWidth:1200, marginInline:'auto', padding:'clamp(6rem,12vw,9rem) clamp(1.25rem,4vw,3rem) clamp(5rem,10vw,8rem)', width:'100%', display:'grid', gridTemplateColumns:'1fr min(480px,45%)', gap:'clamp(3rem,6vw,6rem)', alignItems:'center' }}>
        ...
        <div style={{ animation:'heroFadeIn 0.8s ease both' }}>
          ...
          {/* Status badge */}
          <div style={{ display:'inline-flex', alignItems:'center', gap:'0.5rem', background:'rgba(34,197,94,0.08)', border:'1px solid rgba(34,197,94,0.25)', borderRadius:99, padding:'0.35rem 1rem', marginBottom:'1.75rem', animation:'heroFadeIn 0.6s ease both' }}>
            ...
            <span style={{ color:'#86EFAC', fontSize:'0.8125rem', fontWeight:600 }}>سیستم امنیتی فعال — پشتیبانی ۲۴/۷</span>
          </div>
          ...
          <h1 className="hero-headline" style={{ fontSize:'clamp(2.4rem,5.5vw,4.5rem)', fontWeight:900, lineHeight:1.15, marginBottom:'1.25rem', color:'#fff', letterSpacing:'-0.02em' }}>
            <span style={{ display:'block', animation:'heroFadeIn 0.7s 0.1s ease both', opacity:0, animationFillMode:'forwards' }}>
              امنیت اماکن
            </span>
            <span style={{ display:'block', background:'linear-gradient(90deg, #FDE68A 0%, #F97316 40%, #EA580C 80%, #FDE68A 100%)', backgroundSize:'200% auto', backgroundClip:'text', WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent', animation:'heroFadeIn 0.7s 0.2s ease both, shimmer 3s linear infinite', opacity:0, animationFillMode:'forwards' }}>
              با فناوری روز دنیا
            </span>
            <span style={{ display:'block', color:'rgba(255,255,255,0.75)', fontSize:'0.65em', fontWeight:700, animation:'heroFadeIn 0.7s 0.3s ease both', opacity:0, animationFillMode:'forwards' }}>
              دزدگیر هوشمند بیواز
            </span>
          </h1>

          {/* Subtext */}
          <p style={{ color:'rgba(255,255,255,0.55)', fontSize:'clamp(1rem,1.8vw,1.125rem)', lineHeight:1.8, maxWidth:520, marginBottom:'2.5rem', animation:'heroFadeIn 0.7s 0.4s ease both', opacity:0, animationFillMode:'forwards' }}>
            {cms?.hero_desc ?? 'بیش از ۱۰ سال تجربه در تأمین امنیت منازل، فروشگاه‌ها و اماکن تجاری. نصب رایگان، ضمانت اصالت کالا و پشتیبانی ۲۴ ساعته در سراسر ایران.'}
          </p>
          ...
          <div style={{ display:'flex', gap:'1rem', flexWrap:'wrap', marginBottom:'2.75rem', animation:'heroFadeIn 0.7s 0.5s ease both', opacity:0, animationFillMode:'forwards' }}>
            <a href="tel:02191090" className="hero-cta-primary" style={{ display:'inline-flex', alignItems:'center', gap:'0.6rem', background:'linear-gradient(135deg, #F97316, #EA580C)', color:'#fff', fontWeight:700, fontSize:'1.05rem', padding:'0.9rem 2rem', borderRadius:14, textDecoration:'none', animation:'ctaGlow 2.5s ease-in-out infinite', transition:'transform 0.25s ease, box-shadow 0.25s ease', border:'none', cursor:'pointer' }}>
            ...
            <Link href="/shop" className="hero-cta-secondary" style={{ display:'inline-flex', alignItems:'center', gap:'0.6rem', background:'rgba(255,255,255,0.07)', color:'#fff', fontWeight:600, fontSize:'1.05rem', padding:'0.9rem 2rem', borderRadius:14, textDecoration:'none', backdropFilter:'blur(12px)', border:'1px solid rgba(255,255,255,0.18)', transition:'all 0.25s ease', cursor:'pointer' }}>
              مشاهده محصولات
          ...
          <div className="hero-trust" style={{ display:'flex', gap:'0.6rem', flexWrap:'wrap', animation:'heroFadeIn 0.7s 0.6s ease both', opacity:0, animationFillMode:'forwards' }}>
            {['نصب رایگان', 'ضمانت اصالت', 'پشتیبانی ۲۴/۷', 'گارانتی ۱۸ ماهه'].map((t, i) => (
              <span key={i} className="trust-pill" style={{ display:'inline-flex', alignItems:'center', gap:'0.4rem', background:'rgba(249,115,22,0.08)', border:'1px solid rgba(249,115,22,0.2)', borderRadius:99, padding:'0.3rem 0.85rem', fontSize:'0.8125rem', color:'rgba(255,255,255,0.8)', transition:'all 0.25s ease', cursor:'default' }}>
        ...
        {/* RIGHT — 3D Security Shield visual */}
        <div style={{ display:'flex', alignItems:'center', justifyContent:'center', position:'relative', animation:'heroFadeIn 0.9s 0.3s ease both', opacity:0, animationFillMode:'forwards' }}>
          ...
          <div className="hero-badges" style={{ position:'absolute', display:'flex', flexDirection:'column', gap:'0.75rem', left:-40, top:'50%', transform:'translateY(-50%)' }}>
            {[
              { val:'۵۰۰۰+', lbl:'مشتری', color:'#F97316', delay:0 },
              { val:'۱۰+',   lbl:'سال تجربه', color:'#6080FA', delay:0.3 },
              { val:'۹۸٪',  lbl:'رضایت', color:'#10B981', delay:0.6 },
            ].map((s, i) => (
